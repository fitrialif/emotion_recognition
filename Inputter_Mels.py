...
for fname in name :
    if not os.path.exists('_Features_Mels') :
        os.mkdir('_Features_Mels')

    if not os.path.exists('_Features_Mels/%s'%(fname)) :
        os.mkdir('_Features_Mels/%s'%(fname))

    folders = np.array([])
    index = 0
    for name in glob.glob('_DATA/%s/*'%(fname)) :
        folders = np.append(folders,name)
        a,b = folders[index].split('_DATA/%s'%(fname))
        if not os.path.exists(('_Features_Mels/%s'%(fname)+b)) :
            os.mkdir(('_Features_Mels/%s'%(fname)+b))
        index += 1
    ...
    for label in folders :
        for files in glob.glob(str(label + '/*.wav')) :
            ...
            # str('{:03}'.format(index))
            filename = '_Features_Mels/%s/'%(fname)+b+ '/' + audioname + '.png'
