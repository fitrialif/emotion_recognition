...
for fname in name :
    if not os.path.exists('RAVDESS/_Mels') :
        os.mkdir('RAVDESS/_Mels')

    if not os.path.exists('RAVDESS/_Mels/%s'%(fname)) :
        os.mkdir('RAVDESS/_Mels/%s'%(fname))
    ...
    for name in glob.glob('RAVDESS/%s/*'%(fname)) :
        ...
        if not os.path.exists(('RAVDESS/_Mels/%s'%(fname)+b)) :
            os.mkdir(('RAVDESS/_Mels/%s'%(fname)+b))
        index += 1
    ...
    for label in folders :
        for files in glob.glob(str(label + '/*.wav')) :
            ...
            # str('{:03}'.format(index))
            filename = 'RAVDESS/_Mels/%s/'%(fname)+b+ '/' + audioname + '.png'
